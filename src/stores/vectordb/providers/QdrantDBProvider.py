...
from ..VectorDBEnums import VectorDBEnums, DistanceVectorEnums
from typing import List
import logging 
from qdrant_client import QdrantClient, models

class QdrantDBProvider(VectorDBIntertface):

    def __init__(self, db_path:str, distance_method:str):
        self.db_path = db_path
        self.client = None

        if distance_method == DistanceVectorEnums.COSINE.value:
            self.distance_method = models.Distance.COSINE
        elif distance_method == DistanceVectorEnums.EUCLIDEAN.value:
            self.distance_method = models.Distance.EUCLID
        elif distance_method == DistanceVectorEnums.DOT.value:
    ...
    def insert_one(self, collection_name:str, text:str, vector:list, metadata:dict=None, record_id:str=None):
        if not self.is_collection_exist(collection_name):
            self.logger.error(f"can't insert the Collection {collection_name} does not exist")
            return False
        try:    
    ...
    def insert_many(self, collection_name:str, texts:list, vectors:list,metadata:list=None, record_ids:list=None, batch_size:int=50):
        ...
        for i in  range(0, len(texts), batch_size):
            ...
            try:
                ...
                self.logger.error(f"Error while inserting batch {i} to {batch_end} : {e}")
