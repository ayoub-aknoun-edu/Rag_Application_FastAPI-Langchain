...
class ChunkModel(BaseDataModel):
    ...
    async def create_chunk(self, chunk:DataChunk):
        result = await self.collection.inset_one(chunk.dict(by_alias=True, exclude_unset=True))
    ...
    async def get_project_chunks(self, project_id:ObjectId, page_number:int=1, page_size:int=50):
        result = await self.collection.find(
            {"chunk_project_id": project_id}).skip((page_number-1)*page_size).limit(page_size).to_list(length=None)
        
        return [
            DataChunk(**item) for item in result]
